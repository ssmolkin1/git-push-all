...
if (!shell.which('git')) {
  console.log(
    "Sorry, it looks like you don't have git installed yet. Gal is made for git, so it won't help you unless you have that first. Please install git and try again."
  )
...
// m command merges two branches and pushes to remote
  ...
  const optionDefinitions = [
    ...
    },
    {
      ...
      type: String,
      defaultValue: config.remote,
      description: 'Remote of the "into" branch'
    },
    {
      name: 'no-push',
      alias: 'n',
...
// store command sets up credential storage using libsecret.
  ...
  if (options.help) {
    ...
    if (shell.ls(storePath).indexOf('libsecret') < 0) {
      ...
      if (!shell.which('curl')) {
        console.log(
          'You are not set up yet to store git credentials using libsecret. Gal can set this up for you, but curl is required to install the necessary files. Please install curl and try again.'
        )
        shell.exit(1)
      }
    ...
    if (libsecretContents.indexOf('Makefile') < 0) {
      ...
      shell.exec(
        `sudo curl -o ${storePath}/Makefile https://raw.githubusercontent.com/git/git/master/contrib/credential/libsecret/Makefile`
      )
      console.log('Done!')
    }
    ...
    if (libsecretContents.indexOf('git-credential-libsecret.c') < 0) {
      ...
      shell.exec(
        `sudo curl -o ${storePath}/git-credential-libsecret.c https://raw.githubusercontent.com/git/git/master/contrib/credential/libsecret/git-credential-libsecret.c`
      )
      console.log('Done!')
    }
    ...
    if (libsecretContents.indexOf('git-credential-libsecret.o') < 0) {
      if (shell.exec('ls /lib', { silent: true }).stdout.search(/libsecret/) < 0) {
        console.log(
          'You are not set up yet to store git credentials using libsecret. gal can set this up for you, but libsecret is required. Please install libsecret and try again.'
        )
        shell.exit(1)
      }
...
function printHelpPage(command, optionDefinitions) {
  ...
  const sections = [
    ...
    {
      content:
        "See 'gal <command> -h' for help with " +
        (command ? "another command or 'gal -h' for a full list of commands." : 'a specific command.')
    }
  ]

  if (!command) {
    // Show title and command list only for 'gal -h'
    sections.unshift({
      header: 'Git-push-all',
      content:
        'A git utility to quickly commit and push your entire branch. Also automates setup of git credential storage with libsecret (on Linux Debian-based distros only).'
    })
